function initCards() {
	...
	$(".dish-sub").click(function(e) {
		e.preventDefault();
		// swal({
		// 	title: "Order Placed Successfully!",
		// 	text: "Please wait till it's there at your doorstep.",
		// 	type: "success",
		// 	confirmButtonText: "OK"
		// });
		var products_array = [];
		for (
			var i = 0;
			i < document.getElementsByClassName("dish-row-added").length;
			i++
		) {
			// console.log(
			// 	document.getElementsByClassName("dish-row-added")[i].children
			// );
			products_array.push({
				name: document.getElementsByClassName("dish-row-added")[i].children[0]
					.innerHTML,
				quantity: document.getElementsByClassName("dish-row-added")[i]
					.children[2].children[0].value,
				price: document.getElementsByClassName("dish-row-added")[i].children[3]
					.innerHTML
			});
		}
		...
		$.ajax({
			type: "POST",
			data: { products_array: JSON.stringify(products_array) },
			url: "addToCart.php",
			success: function(d) {
				if (d == 1) {
...
function getHTML(data, i) {
	var img = "../app/adminpanel/images/" + data.image;
	...
	return (
		...
		i +
		'" ><div class="centered">  <img src="../app/cart.png" class="cart-buttonx" id="idx' +
		i +
		'"></div></div><div class = "card-img" style="' +
		styleImg +
...
function initCart() {
	$(".cart-button").click(function() {
		...
		if (row.hasClass("dish-row-hidden")) {
			row.removeClass("dish-row-hidden").addClass("dish-row-added");
		}
	});
	$(".dish-quantity").bind(
...
$(document).ready(function() {
	...
	initFilters();
	$.ajax({
		url: "../app/adminpanel/scripts/getDishData.php",
